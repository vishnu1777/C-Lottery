...
import {Header,Login,Loading, CountdownTimer,AdminControl} from '../components'
...
const Home:NextPage=()=> {
  ...
  const {contract,isLoading,error} = useContract(process.env.NEXT_PUBLIC_LOTTERY_ADDRESS)
  const { data:remainingTickets } = useContractRead(contract, "RemainingTickets")
  const { data:currentWinningReward } = useContractRead(contract, "CurrentWinningReward")
  const { data:ticketPrice } = useContractRead(contract, "ticketPrice")
  const { data:ticketCommission } = useContractRead(contract, "ticketCommission")
  const { data:expiration } = useContractRead(contract, "expiration")
  const { mutateAsync: BuyTickets,isLoading:buyTicketsLoading } = useContractWrite(contract, "BuyTickets")
  ...
  return (
      ...
      <Marquee className='bg-[#0A1F1C] p-5 mb-5' gradient={false} speed={100}>
        <div className='flex space-x-2 mx-10'>
          <h4 className='text-white font-bold'>Last Winner:{lastWinnner ? lastWinnner : "0x0000" }</h4>
      ...
      '>
        ...
        src="https://avatars.githubusercontent.com/u/67725452?v=4" alt="logo" />
